...
$("#subscribeForm").submit(function(event) {
	if (!$('#subscribeForm').validationEngine('validate')) {
		$("#subscribeLoading").attr("style", "display:none");
		$("#subscribeSubmit").show();
		CMS.showNotification("error", "Please fill out the required fields.", "subscribeNotif");
		return false;
	}
	$("#subscribeSubmit").hide();
	$("#subscribeLoading").removeAttr("style");
	...
	$.post(base_url + "newsletter/process/add-new", $("#" + id_form).serialize(), function(data) {
		if (data != 'false') {
			...
			$("#subscribeLoading").attr("style", "display:none");
			$("#subscribeSubmit").show();
			CMS.showNotification("success", "You are now Subscribed to our Newsletter!", "subscribeNotif");
		} else {
			$("#subscribeLoading").attr("style", "display:none");
			$("#subscribeSubmit").show();
			CMS.showNotification("error", "You are already subscribed to our newsletter.", "subscribeNotif");
		}
	});
});
var CMS = {};
CMS.showNotification = function(type, message, id) {
	var html = '';
	$('div#' + id).removeClass();
	$('div#' + id).addClass('alert');
