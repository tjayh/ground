...
jQuery(document).ready(function() {
	"use strict";
		...
		var _sw = '<!-- STYLESWITCHER - REMOVE ON PRODUCTION/DEVELOPMENT -->'
				...
				+ '		<div  id="styleSwitcherNotif" style="display:none;"class="alert alert-mini alert-success margin-bottom-30"><button data-dismiss="alert" class="close" type="button"><span aria-hidden="true">×</span><span class="sr-only">Close</span></button>You successfully updated the styles.</div>'
				+ '			<button onclick="resetSwitcher();" class="btn btn-primary btn-sm">Reset Styles</button> <button  onclick="styleSwticher();" class="btn btn-primary btn-sm">Submit</button>'
				+ '		</div>'
	...
	**/
	...
	if( !jQuery('#style_color_schemes:empty').length ) {
		var color_schemes_val = jQuery('#style_color_schemes').html();
		jQuery("#color_scheme").attr("href", "assets/css/color_scheme/"+color_schemes_val+".css");
	}
	if( !jQuery('#style_color_skin:empty').length ) {
		jQuery('head').append('<link id="css_dark_skin" title="dark" type="text/css" rel="stylesheet" href="assets/css/layout-dark.css">');
		jQuery("#is_dark").trigger('click');
	}
	if( !jQuery('#style_layout_style:empty').length ) {
		var raw_class = jQuery('body').attr('class');
		jQuery("body").attr("class", raw_class+" boxed");
		jQuery("#is_boxed").trigger('click');
	}
	if( !jQuery('#style_layout_rtl:empty').length ) {
		jQuery('head').append('<link id="rtl_ltr" type="text/css" rel="stylesheet" href="assets/css/layout-RTL.css">');
		jQuery("#is_rtl").trigger('click');
	}
	if( !jQuery('#style_patterns:empty').length ) {
		var style_patterns_val = jQuery('#style_patterns').html();
		var raw_class = jQuery('body').attr('class');
		jQuery("body").attr("class", raw_class + " " + style_patterns_val);
	}
	if( !jQuery('#style_boxed_background:empty').length ) {
		var boxed_background_val = jQuery('#style_boxed_background').html();
		jQuery("body").attr("data-background", boxed_background_val);
	}