...

{
	...
	function _getItems($limit = false, $offset = false, $id_item = false)
	{
		...
		if ($id_item) $this->db->where('i.id_promo_item !=', $id_item);
	...
	function _getPromoItem($id_promo_item = false)
	{
		$this->db->select('*, gc.category_title as category');
		$this->db->from('promo_item i');
		$this->db->join('promo_category gc', 'gc.id_promo_category = i.id_promo_category', 'left');
		$this->db->where('i.status', 1);
		if ($id_promo_item) $this->db->where('i.id_promo_item', $id_promo_item);
		$this->db->order_by('i.date DESC');
		$query = $this->db->get();
		if ($query->num_rows() > 0) {
			$result = $query->result_array();
			...
			if ($id_promo_item) return $return[0];
			return $return;
		}
		return false;
	}
