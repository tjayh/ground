...

{
	...
	function _getCategoryList($active = 1, $has_no_parent = false)
	{
		...
		// $this->db->join('events_category gcp', 'gc.id_parent = gcp.id_events_category');
	...
	function _getItems()
	{
		...
		$this->db->from('events_item gi');
		$this->db->join('events_category gc', 'gc.id_events_category = gi.id_events_category');
	...
	{
		$deleteid = $this->input->post('id_events_category');
		if ($deleteid) {
			$this->load->model('core/dbtm_model', 'dbtm');
			...
			if ($result) {
				...
				if ($query->num_rows() > 0) {
					...
					$result = $query->result_array();
					foreach($result as $item) {
						$data['whr_id_events_item'] = $item[id_events_item];
						$params['post_data'] = $data;
