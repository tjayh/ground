...

{
	...
	function _addCategory()
	{
		...
		if($data['id_parent']){
			$this->db->select('*');
			$this->db->from('events_category');
			$this->db->where('id_events_category', $data['id_parent']);
			$parent_query = $this->db->get();
			$parent = $parent_query->row_array();
		...
		if ($data) {
			...
			$data = str_replace("<p><br></p>", "", $data);
			$result = $this->db->insert('events_category', $data);
			if ($result) {
	...
	function _editCategory()
	{
		...
		if ($data) {
			...
			$data = str_replace("<p><br></p>", "", $data);
			$this->db->where($where);
			$result = $this->db->update('events_category', $data);
			if ($result) {
				if (!file_exists($this->upload_dir . $data['category_image_src'])) {
					if (!is_dir($this->upload_dir)) {
						mkdir($this->upload_dir, 0777, TRUE);
