...
CMS.initPage = function(){
	$('a.setFormValues').on('click',function(){
		...
		$('#id_blog_category').val(data.id_parent);
		...
		if(!$(this).hasClass('editItem')){
			$("#id_blog_category").prop('disabled', true);
			$('div#formActions').addClass('hid');
		}else{
			$("#id_blog_category").prop('disabled', false);
		}
		$("#id_blog_category").trigger("liszt:updated");
		CMS.showWidge();
	});
	...
	details[6] = 'Blog category was successfully deleted.'; //success message for delete
	...
	$("#id_blog_category").chosen({allow_single_deselect: true});
	$("#id_blog_category_chzn").css('width','300px');
	$('button.addReset').on('click',function(){
		var flag = true;
		$('select#id_blog_category option').each(function(){
			if(flag){
				$("#id_blog_category").val(0);
				$("#id_blog_category").trigger("liszt:updated");
				flag=false;
			}
		});	
