...
CMS.initPage = function(){
	$('a.setFormValues').on('click',function(){
		...
		$('select#id_faq_category option').each(function(){
			if($(this).val() == data.id_faq_category){	
				...
				$("#id_faq_category").trigger("liszt:updated");
			}
		});	
		if(data.active == 1)
	...
	$("#id_faq_category").chosen();
	$("#id_faq_category_chosen").css('width','300px');//Script somewhere sets div width to zero. This will fix the issue.
	$(".chosen-single").addClass('ignore');
	$('button.addReset').on('click',function(){
		var flag = true;
		$('select#id_faq_category option').each(function(){
			if(flag){
				$("#id_faq_category").val(0);
				$("#id_faq_category").trigger("liszt:updated");
				flag=false;
			}
		});	
...
function changeStatus() {
    ...
    var keyAndVal = "data%5Bwhr_id_faq_item%5D=" + itemID + "&data%5Bclmn_active%5D=" + enableModule;
