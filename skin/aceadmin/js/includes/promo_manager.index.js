...
CMS.initPage = function(){
	...
	$('a.setFormValues').on('click',function(){
		...
		$('[name="where[id_promo_item]"]').val(data.id_promo_item);
		$('#id_promo_category').val(data.id_promo_category);
	...
	details[2] = 'Promo item was successfully created.'; //success message for add
	details[3] = thisURL+thisModule+"/process/edit-item/"; //post url for edit
	details[4] = 'Promo item was successfully updated.'; //success message for edit
	details[5] = thisURL+thisModule+"/process/delete-item/"; //post url for delete
	details[6] = 'Promo item was successfully deleted.'; //success message for delete
	details[7] = 'id_promo_item'; //name of id for delete
	...
	$("#id_promo_category_chzn").css('width','300px');//Script somewhere sets div width to zero. This will fix the issue.
...
function changeStatus() {
    ...
    var keyAndVal = "data%5Bwhr_id_promo_item%5D=" + itemID + "&data%5Bclmn_status%5D=" + enableModule;
        ...
        setTimeout(function() {
            if (data != 'false') {
                ...
                if (dataJ.error != null) CMS.showNotification('error', dataJ.error);
                    ...
                    if (enableModule == 1) {
                        CMS.showNotification('success', 'Promo is successfully Enabled');
                        ...
                        CMS.showNotification('success', 'Promo is successfully Disabled');
